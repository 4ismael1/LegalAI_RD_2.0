...
BEGIN
  ...
  SET 
    ...
    pending_downgrade = false -- Set to false for new subscriptions
  WHERE id = user_id_param;
...
-- Create function to request downgrade (admin only)
...
BEGIN
  ...
  IF NOT (SELECT is_admin()) THEN
    RAISE EXCEPTION 'Only administrators can request subscription downgrades';
  END IF;
